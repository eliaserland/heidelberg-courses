	...
	def __init__(self,L,N1d,sig_v,r_cut,output_frequency):
		...
		for i in range(N1d):
			for j in range(N1d):
				for k in range(N1d):
					#---students---#
					self.coord[ip,i_x] = (0.5+k)*dl 
					self.coord[ip,i_y] = (0.5+j)*dl
					self.coord[ip,i_z] = (0.5+i)*dl
					self.vel[ip,i_x]   = sig_v*rand_gauss()
					self.vel[ip,i_y]   = sig_v*rand_gauss()
					self.vel[ip,i_z]   = sig_v*rand_gauss()
					ip+=1
					#---end---#
